...
int CKafkaProducer::init(char *topic, char *brokers, int partition) {
    ...
    rd_kafka_topic_partition_list_add(m_kafka_topic_partition_list, topic, partition); //Can add more than one topic
...
//	memcpy(buf, str, len);
    ...
    ret = rd_kafka_produce(m_kafka_topic, partition, RD_KAFKA_MSG_F_COPY | RD_KAFKA_MSG_F_FREE, str, len, NULL, 0,
                           NULL);
...
void CKafkaProducer::throttle_cb(rd_kafka_t *rk, const char *broker_name, int32_t broker_id, int throttle_time_ms,
                                 void *opaque) {
    printf("%% THROTTLED %dms by %s (%d)\n", throttle_time_ms, broker_name, broker_id);
}


void CKafkaProducer::offset_commit_cb(rd_kafka_t *rk, rd_kafka_resp_err_t err, rd_kafka_topic_partition_list_t *offsets,
                                      void *opaque) {
    int i;
    int verbosity = 1;
